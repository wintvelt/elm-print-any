module PrintAny exposing (view, viewWithConfig, config, log)
...
-}
import Html exposing (Html, text, p, pre)
...
splitWithChar splitter string =
  ...
  |> List.indexedMap (\ind str -> if (ind > 0) then splitter ++ str else str)
...
splitWithQuotes string =
  String.split "\"" string
  |> List.indexedMap (\i str -> if (i `rem` 2 == 1) then "\"" ++ str ++ "\"" else str)
